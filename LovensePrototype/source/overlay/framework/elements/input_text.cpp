...
bool framework::input_text( const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data )
{
    IM_ASSERT( !( flags & ImGuiInputTextFlags_Multiline ) ); // call InputTextMultiline()
    return input_text_ex( label, NULL, buf, ( int )buf_size, ImVec2( 0, 0 ), flags, callback, user_data );
}

bool framework::input_text_ex( const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data )
{
    ...
    if ( render_cursor || render_selection )
    {
        ...
        // Draw selection
        const ImVec2 draw_scroll = ImVec2( state->Scroll.x, 2.0f );
        if ( render_selection )
        {
        ...
        if ( render_cursor )
        {
            ...
            ImVec2 cursor_screen_pos = ImTrunc( draw_pos + cursor_offset - draw_scroll + ImVec2( 0.f, 2.f ) );
    ...
    else
    {
        ...
        if ( is_multiline || ( buf_display_end - buf_display ) < buf_display_max_length )
        {
            ...
            const ImVec2 draw_scroll = /*state ? ImVec2(state->Scroll.x, 0.0f) :*/ ImVec2( 0.0f, 2.0f ); // Preserve scroll when inactive?
            ImU32 col = GetColorU32( is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text );
            draw_window->DrawList->AddText( g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect );
        }
    }
